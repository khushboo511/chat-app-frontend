...
export async function generateRoomSharedKey(
  ...
  const sharedKeyArrayBuffer = crypto.getRandomValues(new Uint8Array(32));
  ...
  for (const member of room.members) {
    ...
    for (const device of keyBundle.devices) {
      ...
      const { body } = await sessionCipher.encrypt(sharedKeyArrayBuffer.buffer);
      memberKeys.push({
...
export async function encryptMessage(
  ...
  console.log(
    ...
    "shared key during encryption"
  );

  //  Crypto for AES-GCM encryption
...
export async function decryptMessage(
  ...
  if (!ciphertext || !isBase64(ciphertext)) {
    console.error(`Invalid ciphertext for room ${roomId}`);
    return "";
  }
  ...
  try {
    ...
    if (combined.length < 12) {
      console.error(`Ciphertext too short for room ${roomId}`);
      return "";
    }
